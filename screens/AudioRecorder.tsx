...
import { SafeAreaView, TouchableOpacity, View, Text, Alert, Linking, StyleSheet, Dimensions } from 'react-native';
...
// AudioRecorder component for recording, managing, and playing audio files.
const AudioRecorder: React.FC<AudioRecorderProps> = ({
  ...
  // Updates the orientation of the screen and stops the waveform player if orientation changes.
  useEffect(() => {
  ...
  // Handles a timer to update the recording duration.
  useEffect(() => {
    if (recording) {
  ...
  // Updates the waveform display when the recording or selection changes.
  ...
  // Updates the metadata file with details of the recorded audio file.
  const addEntryToMetadata = async (wavFilePath: string) => {
    ...
    try {
      ...
      const metadataEntryPath = `audio/ingredients/${selectedBook}/${selectedChapter}/${selectedChapter}_${selectedVerse}.wav`;
  ...
  // Removes an entry from the metadata file when a recording is deleted.
  ...
  // Requests permission to access the microphone for recording.
  ...
  // Stops the audio recording and processes the final file.
  const stopRecording = async () => {
    try {
      ...
      const outputPath = `${getAudioDirectoryPath()}/${selectedChapter}_${selectedVerse}.m4a`;
  ...
  const convertToWav = async (inputPath: string) => {
    try {
      const outputPath = inputPath.replace(`${selectedChapter}_${selectedVerse}.m4a`, `${selectedChapter}_${selectedVerse}.wav`);
