...
type ImportReferenceCollectionModalProps = {
  visible: boolean;
  onDismiss: () => void;
...
const ImportReferenceCollectionModal: React.FC<
  ImportReferenceCollectionModalProps
> = ({visible, onDismiss, onImport, onUpdate}) => {
    ...
    if (selectedFolder) {
      ...
      try {
        ...
        if (
          ...
          const projectsFolderExists = await RNFS.exists(referenceFolderPath);
  ...
  return (
    <Portal>
      <Dialog visible={visible} onDismiss={handleCancel}>
        <Dialog.Title>Import Reference Project</Dialog.Title>
        <Dialog.Content>
          {isImporting ? (
            <View
              style={{
...
export default ImportReferenceCollectionModal;
